...
VERTEX_MAIN(_EXPORTED_colorRampVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
    ...
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_y, uint);
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_color0, uint);
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_color1, uint);
    uint4 _EXPORTED_a_span = uint4(_EXPORTED_a_spanX, _EXPORTED_a_y, _EXPORTED_a_color0, _EXPORTED_a_color1);
#else
    ...
    float x =
        float((_vertexID & 1) == 0 ? _EXPORTED_a_span.x & 0xffffu : _EXPORTED_a_span.x >> 16) /
        65536.;
